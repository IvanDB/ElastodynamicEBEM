function neuBoundData = BEMenerg_dirN_calcBoundDataNeumann(deltaT, pbParam, domainMesh)
...
switch pbParam.domainType
    case {"barH1", "barH3"}
        ...
        g = @(x, t, n) [0; 0; 2*pbParam.mu * tildeP(x, t) * n];
    otherwise
...
for indTemp = 1 : pbParam.nT
    ...
    parfor indT = 1 : domainMesh.numberTriangles
        gVcurr{indT} = g(domainMesh.center(indT, :), (indTemp - 0.5)*deltaT, domainMesh.normal(indT, 3));
    end
