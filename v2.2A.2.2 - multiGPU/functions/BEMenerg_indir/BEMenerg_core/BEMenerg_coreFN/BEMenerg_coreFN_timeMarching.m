function density = BEMenerg_coreFN_timeMarching(pbParam, domainMesh, methodInfo, GHn, GHw, GHCn, GHCw, DIAGn, DIAGw)
%
...
%Dimensioni problema
numBlocks = BEMenerg_coreFN_calcNumBlocks(pbParam, domainMesh);
...
outPath = strcat("./outputData/", pbParam.domainType, num2str(pbParam.lev), "_FN_", ...
...
constValues = BEMenerg_coreFN_calcCostantData(methodInfo, domainMesh, GHn, GHw, DIAGn, DIAGw);
...
for indIter = 1 : numIter
    ...
    matrixOut = BEMenerg_coreFN_launchKernel(gpuID, kernel, deltaT, pbParam.velP, pbParam.velS, 4*pi*pbParam.rho, ...
    ...
    matrixSubBlocksDIAG = zeros(3, 3, numTriang, numBlocksPerIter);
    ...
    parfor indTemp = 1 : numBlocksPerIter
        ...
        for indBlock = 1 : numTriang
            indRC = 3 * (indBlock - 1);
            matrixSaved{indTemp, indIter}(indRC + (1:3), indRC + (1:3)) = matrixSubBlocksDIAG(:, :, indBlock, indTemp);
        end
...
%Salvataggio matrice (temporaneo per tabelle)
filePath = strcat(outPath, '_matrix');
save(filePath, 'matrixSaved');
