function density = BEMenerg_coreMXGHC_timeMarching(pbParam, domainMesh, methodInfo, GHn, GHw, GHCn, GHCw)
% INPUT 
...
tmpPath = strcat("./tempData/", pbParam.domainType, num2str(pbParam.lev), "_MXGHC_", ...
    num2str(methodInfo.numNodiExt), "_", num2str(methodInfo.numSubRegion), "_", num2str(methodInfo.numNodiSing));
outPath = strcat("./outputData/", pbParam.domainType, num2str(pbParam.lev), "_MXGHC_", ...
    num2str(methodInfo.numNodiExt), "_", num2str(methodInfo.numSubRegion), "_", num2str(methodInfo.numNodiSing));

% CALCOLO PARAMETRI COSTANTI
constValues = BEMenerg_coreMXGHC_calcCostantData(methodInfo, domainMesh, GHn, GHw, GHCn, GHCw);

%% CALCOLO BLOCCO MATRICIALE E0 e BLOCCO VETTORIALE b0 

%Calcolo blocco matriciale E0
[matrix, ~] = BEMenerg_coreMXGHC_calcMatrixBlock(matrix, methodInfo, 0, deltaT, pbParam, domainMesh, constValues);
...
for indStep = 2 : pbParam.nT
    ...
    if maxstep < 0
        ...
        [matrix, isnotzero] = BEMenerg_coreMXGHC_calcMatrixBlock(matrix, methodInfo, indTemp, deltaT, pbParam, domainMesh, constValues);
