__device__ void nucleoTot(double nu[3][3], const double x[3], const double r, const double t, const double cP, const double cS, const double matCoefCurr[3][3], const double vetCoefCurr[3], const double nodoGHcurr[3], const int n);
...
__global__ void kernelV(double *matrix, const double deltaT, const double velP, const double velS, const double const4PiRho,
                          ...
                          const double *vertsT, const double *areeT, const int offsetZ, const int numBlocks, const double maxLen,
                          const double *matCoeff, const double *vetCoeff)  
{
    ...
    extern __shared__ double matrixSubBlock[][9][3];
    int k, i, j, l, n;
    ...
    double matCoefCurr[3][3];
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
            matCoefCurr[i][j] = matCoeff[9*blockIdx.x + 3*j + i];

    double vetCoefCurr[3];
    for (i = 0; i < 3; i++)
        vetCoefCurr[i] = vetCoeff[3*blockIdx.x + i];
    ...
    for(l = 0; l < numPointExt; l++)
    {
        ...
        for (k = 0; k < 3; k++)
        {
            ...
            //Ciclo sui tre blocchi 3x3 verticali
            for (n = 0; n < 3; n++)
            {
                //Calcolo delle 9 componenti del nucleo
                nucleoTot(tempValues, point, pointNorm, istTemp, velP, velS, matCoefCurr, vetCoefCurr, nodoGHcurr, n);
                
                //Somma pesata dei valori del nucleo alla shared memory
                for (i = 0; i < 3; i++)
                    for (j = 0; j < 3; j++)
                        matrixSubBlock[sharedBaseInd][3*n + i][j] += pesoExt * pesoInt * coeffNucleo[k] * tempValues[i][j];
            }
    ...
    if(threadIdx.x == 0 && threadIdx.y == 0)
        for (i = 0; i < 9; i++)
            for (j = 0; j < 3; j++)
            {
                ind = 27*gridDim.x*gridDim.y*blockIdx.z + 9*gridDim.x*(3*blockIdx.y + j) + 9*blockIdx.x + i;
                // matrix[9*blockIdx.x + i][3*blockIdx.y + j][blockIdx.z]
...
__device__ bool isBlockNull(const double *vertsT, const double deltaT, const double cP, const double cS, const int indTemp, const double maxLen)
{
    ...
    double distMax = 0;
    double distMin = 100; //Settare valore più infinto
    double distCurr;
    ...
    return ((indTemp - 1) * cS * deltaT > distMax) || ((indTemp + 1) * cP * deltaT < distMin - maxLen);
}


__device__ void nucleoTot(double nu[3][3], const double x[3], const double r, const double t, const double cP, const double cS, const double matCoefCurr[3][3], const double vetCoefCurr[3], const double nodoGHcurr[3], const int n)
{
    nucleo(nu, x, r, t, cP, cS);

    int i, j;
    
    const double baseFuncValue = matCoefCurr[n][0] * nodoGHcurr[0] +  matCoefCurr[n][1] * nodoGHcurr[1] + matCoefCurr[n][2] * nodoGHcurr[2] + vetCoefCurr[n];

    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
            nu[i][j] *= baseFuncValue;
