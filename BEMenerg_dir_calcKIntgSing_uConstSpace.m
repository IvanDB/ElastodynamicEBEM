...
numNodiBound = methodInfo.blockXbound; %valutare e mettere dinamico

%Estrazione parametri triangolo di campo
matCoeffIntCurr = constValuesF.matCoeff;
vetCoeffIntCurr = constValuesF.vetCoeff;

%Estrazione parametri triangolo sorgente
matCoeffExtCurr = constValuesS.matCoeff;
vetCoeffExtCurr = constValuesS.vetCoeff;

%Inizializzazione variabile integrale corrente
intgTotSing = zeros(9, 9);

%Ciclo sui nodi integrazione esterna
for indNodoExt = 1 : numNodiExt
    %Inizializzazione valore singolo integrale interno
    intgIntSing = zeros(3, 9);

    %Lettura nodo e peso esterni correnti
    nodoExt = constValuesS.GHnodes{indNodoExt};
    pesoExt = constValuesS.GHweights{indNodoExt};
    
    baseFunctionsExtValues = matCoeffExtCurr * nodoExt' + vetCoeffExtCurr;
    ...
    for indChild = 1 : 3
        for indDiag = 1 : numNodiDiag                            
            ...
            baseFunctionsIntValues = (matCoeffIntCurr * nodoInt' + vetCoeffIntCurr)';
            ...
            nucleoKR = zeros(3, 9);
            for m = 1 : 3
                vettRcoeffCurr = cross(normInt, matCoeffIntCurr(m, :));
                nucleoKR(:, 3*(m-1) + (1:3)) = -nuKsmRj(vettR, lungR, diffTemp, 1) .* vettRcoeffCurr(1) - nuKsmRj(vettR, lungR, diffTemp, 2) .* vettRcoeffCurr(2) - nuKsmRj(vettR, lungR, diffTemp, 3) .* vettRcoeffCurr(3);
            end

            %Calcolo somma dei nuclei 
            somNucleiZeta = nucleoKR + kron(baseFunctionsIntValues, nuKL(vettR, lungR, normInt, diffTemp) + nuKT(vettR, lungR, normInt, diffTemp));
    ...
    for indL = 1 : 3
        ...
        for indS = 1 : numNodiBound
            ...
            lungR = norm(vettR, 2);
            baseFunctionsIntValueStart = (matCoeffIntCurr * points(indS, :)' + vetCoeffIntCurr)';
            valStart = kron(baseFunctionsIntValueStart, nuKsmRj(vettR, lungR, diffTemp, 1) .* vettTang(1) + nuKsmRj(vettR, lungR, diffTemp, 2) .* vettTang(2) + nuKsmRj(vettR, lungR, diffTemp, 3) .* vettTang(3));


            vettR = nodoExt - points(indS+1, :);
            lungR = norm(vettR, 2);
            baseFunctionsIntValueEnd = (matCoeffIntCurr * points(indS+1, :)' + vetCoeffIntCurr)';
            valEnd = kron(baseFunctionsIntValueEnd, nuKsmRj(vettR, lungR, diffTemp, 1) .* vettTang(1) + nuKsmRj(vettR, lungR, diffTemp, 2) .* vettTang(2) + nuKsmRj(vettR, lungR, diffTemp, 3) .* vettTang(3));
    ...
    intgTotSing = intgTotSing + pesoExt .* kron(baseFunctionsExtValues, intgIntSing + intgBoundSing);
end
return