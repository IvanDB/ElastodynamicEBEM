...
__global__ void kernelPPnewPlayer(double *uRawP, const double velP, const double velS, const double lambda, const double mu, const double const4PiRhoDeltaT,
                         ...
                         const int numNodes, const int *indSMmatrix, const double *matCoeff, const double *vetCoeff, const int *numTosS, const int *indTonS)  
{
    ...
    //Lettura istante temporale associato al thread       
    ...
    for(p = 0; p < numNodes; p++)
    {
        ...
        int numTonCurrS = numTosS[p];
       
        //Ciclo sui triangoli di campo associati a questo nodo
        for(m = 0; m < numTonCurrS; m++)
        {            
            //Estrazione indice del triangolo corrente
            int indT = indTonS[baseIndFlag + m] - 1;
            
            //Estrazione indice vertice-triangolo corrente
            int indSMcurr = indSMmatrix[baseIndFlag + indT];
            ...
            if (isSubBlockNull(centerT, maxLenT, sourcePoint, velP, velS, currT, currT, indT))
                continue;
            ...
            for(n = 0; n < numSubRegion; n++)
            {
                ...
                for(l = 0; l < numSubPoint; l++)
                {
                    ...
                    double pesoPP = stdPPw[l] * areeT[indT];
        ...
        //Lettura componenti delle densità/dei dati relativi al blocco corrente
        ...
        if(threadIdx.x > 0)
        {
            vettLinCurr[0] -= 2 * vettLin[(3*numNodes)*(threadIdx.x - 1) + 3*p + 0];
            vettLinCurr[1] -= 2 * vettLin[(3*numNodes)*(threadIdx.x - 1) + 3*p + 1];
            vettLinCurr[2] -= 2 * vettLin[(3*numNodes)*(threadIdx.x - 1) + 3*p + 2];
        }

        if(threadIdx.x > 1)
        {
            vettLinCurr[0] += vettLin[(3*numNodes)*(threadIdx.x - 2) + 3*p + 0];
            vettLinCurr[1] += vettLin[(3*numNodes)*(threadIdx.x - 2) + 3*p + 1];
            vettLinCurr[2] += vettLin[(3*numNodes)*(threadIdx.x - 2) + 3*p + 2];
        }
