function sol = BEMenerg_timeMarching_FN(pb_param, domainMesh, gha, ghw, dgn, dgw, ghcn, ghcw, typeQuad)
% INPUT 
...
nExt = pb_param.ngh;
nInt = pb_param.ngF * strcmp(typeQuad, "DQM") + pb_param.nghF * strcmp(typeQuad, "DGH") + pb_param.nghc * (strcmp(typeQuad, "DQC") + strcmp(typeQuad, "DQC3"));

%Inizializzazione path directory temp e output
tmpPath = strcat('./temp/', pb_param.domain_type, num2str(pb_param.lev), '_', typeQuad, '_', num2str(nExt), '_', num2str(nInt));
outPath = strcat('./output/', pb_param.domain_type, num2str(pb_param.lev), '_', typeQuad, '_', num2str(nExt), '_', num2str(nInt));

%% CALCOLO BLOCCO MATRICIALE E0 e BLOCCO VETTORIALE b0 

%Calcolo blocco matriciale E0
[matrix, ~] = BEMenerg_calcMatrixBlock_FN(matrix, hk, dt, pb_param, domainMesh, gha, ghw, dgn, dgw, ghcn, ghcw, typeQuad);
...
for ind_step = 2 : pb_param.Nt
    ...
    if(maxstep == 0)
        ...
        [matrix, isnotzero] = BEMenerg_calcMatrixBlock_FN(matrix, hk, dt, pb_param, domainMesh, gha, ghw, dgn, dgw, ghcn, ghcw, typeQuad);
