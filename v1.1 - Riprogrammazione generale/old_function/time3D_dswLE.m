function ris=time3D_dswLE(pb_param,TS,areaS,TF,vnF,thk,ngaush,gha,ghw)
% function ris=time3D_dswLE(pb_param,TS,vnS,areaS,curlS,TF,vnF,areaF,curlF,...
%     thk,dt,ngaush,gha,ghw)
...
%  temporali in corrispondenza della quale si vuole calcolare
%  il blocco matriciale 
%
...
%  in corrispondenza di una fissata differenza temporale tkh
%
%-------------------------------------------------------------------------

% %Inizializzazione della matrice 3x3 rist che conterrà il risultato della 
% %doppia integrazione in spazio 
% ris = zeros(3,3);
...
ris5 = zeros(3,3); %Integrale di 1/r (Onda P e S)
ris6 = zeros(3,3); %Integrale di r_i*r_j/r^3 (Onda P e S)
...
%     hold on
%     fill3([TF(1,1) TF(2,1) TF(3,1)],[TF(1,2) TF(2,2) TF(3,2)],[TF(1,3) TF(2,3) TF(3,3)],'r')
%     hold on
...
%     %Memorizziamo nella riga i-esima della matrice 3x3 TF2 le componenti 
%     %del VETTORE DISTANZA tra il VERTICE P_i del TRIANGOLO di CAMPO e 
%     %il PUNTO SORGENTE
%     TF2(1:3,:)= TF(1:3,:)-sp; 
    
%     %VETTORE DISTANZA tra il VERTICE P_1 del triangolo di campo e il
%     %PUNTO SORGENTE
%     TF2(1,:)=TF(1,:)-sp;
%     %VETTORE DISTANZA tra il VERTICE P_2 del triangolo di campo e il
%     %PUNTO SORGENTE
%     TF2(2,:)=TF(2,:)-sp;
%     %VETTORE DISTANZA tra il VERTICE P_3 del triangolo di campo e il
%     %PUNTO SORGENTE
%     TF2(3,:)=TF(3,:)-sp;
...
%             hold on            
%             t =linspace(0,2*pi);
%             x1_P = sp(1)+pR_P*cos(t);
%             y1_P= sp(2)+pR_P*sin(t);
%             plot3(x1_P,y1_P,zeros(1,100),'r','linewidth',1.5)            
%             x1_S = sp(1)+pR_S*cos(t);
%             y1_S= sp(2)+pR_S*sin(t);
%             plot3(x1_S,y1_S,zeros(1,100),'k','linewidth',1.5)
...
%     %ROTAZIONE finale (per ora valida solo per la screen)
%     %Calcoliamo l'angolo che il versore e1 del sistema di riferimento
%     %bidimensionale fissato forma rispetto al semiasse positivo
%     %dell'asse delle ascisse in R3
%     theta0 = atan2(Sistema_rif.e1(2),Sistema_rif.e1(1));
%     %Determiniamo l'angolo di rotazione
%     if (theta0<0)
%         theta0=abs(theta0);
%     else
%         theta0=2*pi-theta0;
%     end
%     %Applichiamo la rotazione inversa
%     rist2=rotazione_inversa_attorno_asse_z(rist2,theta0);
%     rist4=rotazione_inversa_attorno_asse_z(rist4,theta0);
%     rist6=rotazione_inversa_attorno_asse_z(rist6,theta0);
...
%     [I5,I6]=Prova(pb_param,sp(1),sp(2),thk);
%     D5=abs(I5-rist5)
%     D6=abs(I6-rist6)
    
    %FORMULA di QUADRATURA di GAUSS-HAMMER per l'INTEGRALE 
    %ESTERNO
    %Moltiplichiamo gli integrali analitici trovati in corrispondenza 
    ...
    %INTEGRALE di 1/r (onda P)
    ris1 = ris1+rist1*ghw(ngaush,indGH);
    %INTEGRALE di r_i*r_j/r^3  (onda P)          
    ris2 = ris2+rist2*ghw(ngaush,indGH);
    %INTEGRALE di 1/r^3 (onda P)
    ris3 = ris3+rist3*ghw(ngaush,indGH);
    %INTEGRALE di r_i*r_j/r^5 (onda P)
    ris4 = ris4+rist4*ghw(ngaush,indGH);
    %INTEGRALE di 1/r (onda P e S)
    ris5 = ris5+rist5*ghw(ngaush,indGH);
    %INTEGRALE di r_i*r_j/r^3 (onda P e S)
    ris6 =ris6+rist6*ghw(ngaush,indGH);
    
    
end %fine ciclo su indGH (nodi di Gauss-Hammer)

%Memorizziamo nella matrice 3x3 ris il RISULTATO della DOPPIA INTEGRAZIONE 
%in SPAZIO sul triangolo sorgente corrente (integrazione esterna) e sul 
%triangolo di campo corrente (integrazione interna analitica)

ris = ((ris1-ris2)/pb_param.velP^2+thk^2*(-ris3+3*ris4)+((pb_param.velP^2+pb_param.velS^2)*ris5...
    +(pb_param.velP^2-pb_param.velS^2)*ris6)/(pb_param.velP*pb_param.velS)^2)/2; 
