function sol = time3D_history(pb_param,domainMesh,gha,ghw)
...
%[matrix,rhs,iok,mat_screen0] = time3D_syst(matrix,rhs,hk,dt,pb_param,domainMesh,gha,ghw,maxstep);
[matrix,rhs,~] = time3D_syst(matrix,rhs,hk,dt,pb_param,domainMesh,gha,ghw,maxstep);

%Nome del file in cui verrà memorizzata la matrice corrente
...
for ind_step = 2:pb_param.Nt
    ...
    if(maxstep==0) 
        ...
        [matrix,rhs,iok] = time3D_syst(matrix,rhs,hk,dt,pb_param,domainMesh,gha,ghw,maxstep);
%        [matrix,rhs,iok,mat_screen0] = time3D_syst(matrix,rhs,hk,dt,pb_param,domainMesh,gha,ghw,maxstep);
