function G = BEMenerg_coreGPUdiag_coeffG_TC(rhoS, THETA1, THETA2, infoTri2D)
% INPUT
% - 
% - 
% OUTPUT
...
% Calcolo COEFFICIENTI G_i (vedi Milroy)
G(1) = (F * g_3) - (rhoS * THETA1tilde);

G(4) = 1 / F * (cos(THETA2 + beta) ...
                - cos(THETA1 + beta)) ...
       + 1 / rhoS * THETA1tilde;

G(7) = -rhoS * Phi(5) + F * (cos(alpha - gamma + THETA2) ...
                           - cos(alpha - gamma + THETA1) ...
                           + sin_alpha^2 * g_3);

G(8) = -rhoS * Phi(4) + F * (cos(THETA1 + alpha) ...
                           - cos(THETA2 + alpha) ...
                           - sin_alpha * sin_beta * g_3);

G(9) = -rhoS * Phi(3) + F * (cos(beta - THETA1) ...
                           - cos(beta - THETA2) ...
                           + sin_beta^2 * g_3);
