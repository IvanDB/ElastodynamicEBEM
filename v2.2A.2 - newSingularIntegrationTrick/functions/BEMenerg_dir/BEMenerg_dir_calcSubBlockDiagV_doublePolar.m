function ris = BEMenerg_dir_calcSubBlockDiagV_doublePolar(pbParam, methodInfo, indTemp, deltaT, constValuesBlock, DIAGn, DIAGw)
...
minDiffTemp = istTemp(3) * deltaT; % 0; %
...
for var = 1 : 3
    ...
    for indGH = 1 : nGH
        ...
        for indChild = 1 : 3
            [nodi3D{1}, pesi3D{1}] = generatePolarG2Dnodes(constValuesBlock.DIAGverts{indGH, indChild}, cS * minDiffTemp, cS * currT, sqrt(methodInfo.numNodiDiag), sqrt(methodInfo.numNodiDiag));
            [nodi3D{2}, pesi3D{2}] = generatePolarG2Dnodes(constValuesBlock.DIAGverts{indGH, indChild}, cS * currT, cP * currT, sqrt(methodInfo.numNodiDiag), sqrt(methodInfo.numNodiDiag));
            for indG2DC = 1 : length(pesi3D{1})
                for indSec = 1 : 2
                    nodoInt = nodi3D{indSec}(indG2DC, :);
                    pesoInt = pesi3D{indSec}(indG2DC);
    
                    vettX = nodoInt - nodoExt;
                    lungX = norm(vettX);
    
                    intgG2DC = intgG2DC + pesoInt .* nu(vettX, lungX, currT);
                end
            end
        end
