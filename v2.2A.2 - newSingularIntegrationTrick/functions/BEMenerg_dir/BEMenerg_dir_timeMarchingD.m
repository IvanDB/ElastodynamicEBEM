function density = BEMenerg_dir_timeMarchingD(pbParam, domainMesh, methodInfo, GHn, GHw, GHCn, GHCw, DIAGn, DIAGw)
%
...
for indIter = 1 : numIterV
    ...
    for indTemp = 1 : numBlocksPerIterV
        ...
        parfor indBlock = 1 : numTriang
            ...
            % matrixSubBlocksDIAG(:, :, indBlock, indTemp) = BEMenerg_dir_calcSubBlockDiagV_polar(pbParam, methodInfo, istTemp, deltaT, constValues{indBlock}, DIAGn, DIAGw);
            % matrixSubBlocksDIAG(:, :, indBlock, indTemp) = BEMenerg_dir_calcSubBlockDiagV_doublePolar(pbParam, methodInfo, istTemp, deltaT, constValues{indBlock}, DIAGn, DIAGw);
            % matrixSubBlocksDIAG(:, :, indBlock, indTemp) = BEMenerg_dir_calcSubBlockDiagV_rescal(pbParam, methodInfo, istTemp, deltaT, constValues{indBlock}, DIAGn, DIAGw);
            % matrixSubBlocksDIAG(:, :, indBlock, indTemp) = BEMenerg_dir_calcSubBlockDiagV_cond(pbParam, methodInfo, istTemp, deltaT, constValues{indBlock}, DIAGn, DIAGw);
            matrixSubBlocksDIAG(:, :, indBlock, indTemp) = BEMenerg_dir_calcSubBlockDiagV_final(pbParam, methodInfo, istTemp, deltaT, constValues{indBlock}, DIAGn, DIAGw);
        end
        timeTest(1) = toc(timerTest);
        timerTest = tic;
        parfor indBlock = 1 : numTriang
            matrixSubBlocksDIAG_c(:, :, indBlock, indTemp) = BEMenerg_dir_calcSubBlockDiagV_final_externC(pbParam, methodInfo, istTemp, deltaT, constValues{indBlock}, DIAGn, DIAGw);
        end
        timeTest(2) = toc(timerTest);

        disp("Done V diag n:" + indTemp + " in " + timeTest(1) + "s [m] / " + timeTest(2) + "s [c]");
        disp("Err = " + max(abs(matrixSubBlocksDIAG(:, :, :, indTemp) - matrixSubBlocksDIAG_c(:, :, :, indTemp)), [], "all"));
    end
...
for indIter = 1 : numIterK
    %Avvio computazione GPU
    matrixOut = BEMenerg_dir_launchKernelK(gpuID, kernelK, deltaT, pbParam.velP, pbParam.velS, pbParam.lambda, pbParam.mu, pbParam.rho, 4*pi*deltaT, ...
                                                    stdGHw, stdGHnx, stdGHny, stdGHnz, methodInfo.numNodiExt, ...
                                                    stdGHCw, stdGHCnx, stdGHCny, stdGHCnz, ...
    ...
    for indTemp = 1 : numBlocksPerIterK
        ...
        parfor indRow = 1 : numTriang
            for indV = 1 : 3
                matrixSubBlocksSING_c(:, :, indV, indRow, indTemp) = BEMenerg_dir_calcSubBlockSingK_externC(methodInfo, deltaT, pbParam, domainMesh, DIAGn, DIAGw, indTempCurr, indRow, constValues{indRow}, indV);
            end
        end
        timeTest(2) = toc(timerTest);
        disp("Done K sing n:" + indTemp + " in " + timeTest(1) + "s [m] / " + timeTest(2) + "s [c]");
        disp("Err = " + max(abs(matrixSubBlocksSING(:, :, :, :, indTemp) - matrixSubBlocksSING_c(:, :, :, :, indTemp)), [], "all"));
    end
...
betaK = BEMenerg_dir_calcBetaKgpu(deltaT, pbParam, domainMesh, matrixSavedK);
% betaK = BEMenerg_core_calcBetaKcpu(methodInfo, deltaT, pbParam, domainMesh, constValues, DIAGn, DIAGw);
