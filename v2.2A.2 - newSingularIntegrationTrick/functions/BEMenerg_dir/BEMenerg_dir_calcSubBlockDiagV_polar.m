function ris = BEMenerg_dir_calcSubBlockDiagV_polar(pbParam, methodInfo, indTemp, deltaT, constValuesBlock, DIAGn, DIAGw)
...
minDiffTemp = 0; istTemp(3) * deltaT;
...
for var = 1 : 3
    ...
    for indGH = 1 : nGH
        ...
        for indChild = 1 : 3
            [nodi3D, pesi3D] = generatePolarG2Dnodes(constValuesBlock.DIAGverts{indGH, indChild}, cS * minDiffTemp, cP * currT, sqrt(methodInfo.numNodiDiag), sqrt(methodInfo.numNodiDiag));
            for indG2DC = 1 : length(pesi3D) 
                nodoInt = nodi3D(indG2DC, :);
                pesoInt = pesi3D(indG2DC);
