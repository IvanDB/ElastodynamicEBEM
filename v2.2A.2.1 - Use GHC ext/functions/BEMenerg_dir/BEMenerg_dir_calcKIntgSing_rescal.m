function intgTotSing = BEMenerg_dir_calcKIntgSing_rescal(methodInfo, pbParam, diffTemp, vettVMS, indVsmCurr, constValuesS, constValuesF, normInt, DIAGn, DIAGw, minDiffTemp)
...
for indNodoExt = 1 : numNodiExt
    ...
    for indChild = 1 : 3
        nodiInt = DIAGn * constValuesF.DIAGverts{indNodoExt, indChild};
        pesiInt = DIAGw * constValuesF.DIAGarea{indNodoExt, indChild};

        v1 = constValuesF.DIAGverts{indNodoExt, indChild}(1, :);
        v2 = constValuesF.DIAGverts{indNodoExt, indChild}(2, :);
        v3 = constValuesF.DIAGverts{indNodoExt, indChild}(3, :);

        l21 = norm(v2 - v1);
        hTriangle = 2 * constValuesF.DIAGarea{indNodoExt, indChild} / l21;
        radius = cP * diffTemp;
        rescalFactor = min(1, radius / hTriangle);

        nodiInt = v3 + (nodiInt - v3) * rescalFactor;
        pesiInt = pesiInt * (rescalFactor^2);

        % plotNodes(constValuesF.DIAGverts{indNodoExt, indChild}, nodiInt, ".", 5, 200);

        for indDiag = 1 : numNodiDiag                            
            %Calcolo nodo e peso corrente
            nodoInt = nodiInt(indDiag, :);
            pesoInt = pesiInt(indDiag);
