function intgTotSing = BEMenerg_dir_calcKIntgSing_final_externC(methodInfo, pbParam, diffTemp, vettVMS, indVsmCurr, constValuesS, constValuesF, normInt, DIAGn, DIAGw, minDiffTemp)
...
for indNodoExt = 1 : numNodiExt
    ...
    for indChild = 1 : 3
        minRad = max(cS * minDiffTemp, 0);
        [nodiInt, pesiInt] = generateFinalG2Dnodes_opt(constValuesF.DIAGverts{indNodoExt, indChild}, minRad, cS * diffTemp, cP * diffTemp, sqrt(numNodiDiag), sqrt(numNodiDiag));
          

        % plotNodes(constValuesF.DIAGverts{indNodoExt, indChild}, nodiInt, ".", 5, 200);

        intgIntSing = intgIntSing + kernelK_dfor(length(pesiInt), nodiInt, pesiInt, nodoExt, diffTemp, cP, cS, lambda, mu, rho, ...
                                                    vettVMS, matCoeffCurr, vetCoeffCurr, indVsmCurr, normInt);
        
        % for indDiag = 1 : length(pesiInt)                            
        ...
        % 
        %     temp = temp + pesoInt .* somNucleiZeta;
