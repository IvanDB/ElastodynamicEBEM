function intgTotSing = BEMenerg_dir_calcKIntgSing_doublePolar(methodInfo, pbParam, diffTemp, vettVMS, indVsmCurr, constValuesS, constValuesF, normInt, DIAGn, DIAGw, minDiffTemp)
...
for indNodoExt = 1 : numNodiExt
    ...
    for indChild = 1 : 3
        [tempNodes{1}, tempWeights{1}] = generatePolarG2Dnodes(constValuesF.DIAGverts{indNodoExt, indChild}, cS * minDiffTemp, cS * diffTemp, sqrt(numNodiDiag), sqrt(numNodiDiag));
        [tempNodes{2}, tempWeights{2}] = generatePolarG2Dnodes(constValuesF.DIAGverts{indNodoExt, indChild}, cS * diffTemp, cP * diffTemp, sqrt(numNodiDiag), sqrt(numNodiDiag));
        nodi3D = [tempNodes{1}; tempNodes{2}];
        pesi3D = [tempWeights{1}; tempWeights{2}];

        for indDiag = 1 : length(pesi3D)                         
            %Calcolo nodo e peso corrente
            nodoInt = nodi3D(indDiag, :);
            pesoInt = pesi3D(indDiag);
