function density = BEMenerg_coreFN_timeMarchingTESTtot(pbParam, domainMesh, methodInfo, GHn, GHw, GHCn, GHCw, DIAGn, DIAGw)
%
...
% Setup kernel
kernelK = parallel.gpu.CUDAKernel("kernelKcondMODlite.ptx", "kernelKcondMODlite.cu");
...
for indTemp = 1 : pbParam.nT
    ...
    for indN = 1 : domainMesh.number_nodes
        gKcurr{indN} = [0; 0; tildeU(domainMesh.coordinates(indN, :), indTemp*deltaT)];
    end
...
matrixKfull = cell(pbParam.nT, pbParam.nT);
matrixVfull = cell(pbParam.nT, pbParam.nT);
for indR = 1 : pbParam.nT
    for indC = 1 : indR
        matrixKfull{indR, indC} = full(matrixSavedK{indR - indC + 1});
        matrixVfull{indR, indC} = full(matrixSavedV{indR - indC + 1});
    end
    for indC = (indR + 1) : pbParam.nT
        matrixKfull{indR, indC} = zeros(3*numTriang, 3*numNodes);
        matrixVfull{indR, indC} = zeros(3*numTriang, 3*numTriang);
    end
end
matrixKfull = cell2mat(matrixKfull);
matrixVfull = cell2mat(matrixVfull);

matrixIfull = cell(pbParam.nT, pbParam.nT);
for indR = 1 : pbParam.nT
    for indC = 1 : pbParam.nT
        if indR == indC
            matrixIfull{indR, indC} = matrixIGamma;
        elseif indR == (indC + 1)
            matrixIfull{indR, indC} = -matrixIGamma;
        else
            matrixIfull{indR, indC} = zeros(3*numTriang, 3*numNodes);
        end
    end
end
matrixIfull = cell2mat(matrixIfull);
matrixIstar = 2.*matrixIfull';

matrixSist = [matrixKfull + matrixIfull, -matrixVfull; ...
                zeros(3*numNodes*pbParam.nT, 3*numNodes*pbParam.nT), matrixIstar];

tnSist = [zeros(3*numTriang*pbParam.nT, 1); matrixIstar * gV];

solSist = matrixSist \ tnSist;

test = matrixSist * [gK; gV];
disp(max(abs(tnSist - test)));

density = reshape(solSist, [3*(numNodes + numTriang), pbParam.nT]);
