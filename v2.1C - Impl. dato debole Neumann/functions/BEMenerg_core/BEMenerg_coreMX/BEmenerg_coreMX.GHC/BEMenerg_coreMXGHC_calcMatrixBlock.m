function [matrix, isnotzero] = BEMenerg_coreMXGHC_calcMatrixBlock(matrix, methodInfo, indTemp, deltaT, pbParam, domainMesh, constValues)
%INPUT
...
parfor ind = 1 : numT^2
    ...
    if(((indTemp-1) * pbParam.velS * deltaT < distMax) && ((indTemp + 1) * pbParam.velP * deltaT >= distMin))
        ...
        if(indS == indF)
            ...
            matrixSubBlock = BEMenerg_coreMXGHC_calcMatrixSubBlock(pbParam, methodInfo, indTemp, deltaT, ...
                                                                constValues{indS}.GHnodes, constValues{indS}.GHweights, ...
                                                                constValues{indF}.GHCnodes, constValues{indF}.GHCweights);
        end
