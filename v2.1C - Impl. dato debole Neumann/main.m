...
switch methodInfo.typeIntg
    ...
    case "FN"
        ...
        density = BEMenerg_coreFN_timeMarchingTEST(pbParam, domainMesh, methodInfo, EXTn, EXTw, INTn, INTw, DIAGn, DIAGw);
        %density = BEMenerg_coreFN_timeMarchingTESTtot(pbParam, domainMesh, methodInfo, EXTn, EXTw, INTn, INTw, DIAGn, DIAGw);
end
densityU = density(1 : (3*domainMesh.number_nodes), :);
densityP = density(3.*domainMesh.number_nodes + (1 : (3*domainMesh.numberTriangles)), :);
%Plot della densità ottenuta
glbIndexFigures = plotSolFormDirNeumann(pbParam, domainMesh, densityU, glbIndexFigures);
glbIndexFigures = plotDensity(pbParam, domainMesh, densityP, glbIndexFigures);
