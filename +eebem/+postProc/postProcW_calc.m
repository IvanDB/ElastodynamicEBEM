function uXT = postProcW_calc(pbParam, domainMesh, density, methodInfo, x, t, stdPPnx, stdPPny, stdPPnz, stdPPw, kernelP, matCoeff, vetCoeff, indSMmatrixGPU, vertsT, areasT, normT)
import eebem.postProc.*
%% GPU SETUP 

gpuID = gpuDevice;
%% memory before calculations
% memBefore = gpuID.AvailableMemory;
...
% memAfter = gpuID.AvailableMemory;
% fprintf("t=%.3f nHat=%d | mem before=%.2f MB after=%.2f MB delta=%.2f MB\n", ...
    % t, nHat, memBefore/1e6, memAfter/1e6, (memBefore-memAfter)/1e6);
return