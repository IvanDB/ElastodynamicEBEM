...
__global__ void kernelK(double* __restrict__ matrix, const double deltaT, const double velP, const double velS, const double lambda, const double mu, const double rho, const double const4PiDeltaT,
                          const double* __restrict__ stdExtW, const double* __restrict__ stdExtNx, const double* __restrict__ stdExtNy, const double* __restrict__ stdExtNz, const size_t numExtNodes,
                          ...
                          const int offsetZ, const int numBlocks, const double* __restrict__ nodesMesh, const double maxLen)  
{
    ...
    const size_t baseFlagIdx = blockIdx.y * gridDim.x;
    ...
    { 
        ...
        const size_t indSMcurr = indSMmatrix[baseFlagIdx + m];
        if(indSMcurr == 0)
            continue;
        ...
        for(size_t l = 0; l < numExtNodes; ++l)
        {
            //Source weight
