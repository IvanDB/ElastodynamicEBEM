...
postCUDAkernelsDirectory = fullfile(basePath, "+eebem", "+postProc", "kernelsCUDA");
postCUDAkernelsFileNames = ["kernelG.cu", "kernelP.cu"];

for CUDAkernel = fullfile(postCUDAkernelsDirectory, postCUDAkernelsFileNames)
    cmd = strcat(scriptCompiler, ' nvcc -ptx -O3 -Wno-deprecated-gpu-targets -arch=compute_', gpuCC, ' -odir "', binOutputDirectory, '" "', CUDAkernel, '"');
    [status, cmdout] = system(cmd);
    assert(~status, "CUDA compilation failed! Shell output:" + newline + cmdout)
end
