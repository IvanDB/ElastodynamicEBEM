function glbIndexFigures = plotDensityT(pbParam, domainMesh, density, glbIndexFigures)

    indComp{1} = kron(ones(1, domainMesh.numberTriangles), 1 : 3) + kron(9 * (0 : (domainMesh.numberTriangles - 1)), ones(1, 3));
    indComp{2} = kron(ones(1, domainMesh.numberTriangles), 4 : 6) + kron(9 * (0 : (domainMesh.numberTriangles - 1)), ones(1, 3));
    indComp{3} = kron(ones(1, domainMesh.numberTriangles), 7 : 9) + kron(9 * (0 : (domainMesh.numberTriangles - 1)), ones(1, 3));
    densityOnT = (density(indComp{1}, :) + density(indComp{2}, :) + density(indComp{3}, :)) ./ 3;
    ...
    switch pbParam.domainType
        ...
        case 'barH1'
            nDim = 3;
            tVal = pbParam.nT .* (1 : 4 : pbParam.Tfin) ./ pbParam.Tfin ;
            jVal = 3; %1 : 
        case 'barH3'
            nDim = 3;
    ...
    for i = tVal
        for j = jVal
            ...
            %plot dati
            if(nDim == 3)
                fill3(X, Y, Z, densityOnT(j:3:end, i));
            elseif(nDim == 2)
                fill(X, Y, densityOnT(j:3:end, i));
            end
