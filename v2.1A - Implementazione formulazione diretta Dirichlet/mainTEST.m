...
listProblems = ["input_barH1_mid_base.txt", "input_barH1_mid_delta2.txt", "input_barH1_mid_delta4.txt", ...
                ...
                "input_barH1_mid_4delta.txt", "input_barH3_mid_4delta.txt"
               ];
...
switch methodInfo.typeIntg
    ...
    case "FN"
        density = BEMenerg_dirD_timeMarching(pbParam, domainMesh, methodInfo, EXTn, EXTw, INTn, INTw, DIAGn, DIAGw);
end
...
switch pbParam.domainType
    case "barH1"
        indR = 3.*[71, 73, 87, 89];
    case "barH3"
        indR = 3.*[34, 35, 36, 37, 38, 40];
end
tVal = (1 : pbParam.nT) .* (pbParam.Tfin ./ pbParam.nT);
plot(tVal, mean(density(indR, :), 1))
