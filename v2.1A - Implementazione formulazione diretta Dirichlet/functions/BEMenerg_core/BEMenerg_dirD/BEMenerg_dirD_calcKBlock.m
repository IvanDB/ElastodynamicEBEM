function KBlock = BEMenerg_dirD_calcKBlock(methodInfo, indTemp, deltaT, pbParam, domainMesh, constValues, DIAGn, DIAGw)
...
for ind = 1 : numT * numS
    ...
    for indZeta = 1 : 4
        ...
        for indM = 1 : numT
            ...
            if indM ~= indMtilde
                intgTotSing = BEMenerg_dirD_calcKIntgNotSing(methodInfo, pbParam, diffTemp, vettVMS, indVsmCurr, constValuesS, constValuesF, normInt);
            end
            if indM == indMtilde
                intgTotSing = BEMenerg_dirD_calcKIntgSing(methodInfo, pbParam, diffTemp, vettVMS, indVsmCurr, constValuesS, constValuesF, normInt, DIAGn, DIAGw);
            end
