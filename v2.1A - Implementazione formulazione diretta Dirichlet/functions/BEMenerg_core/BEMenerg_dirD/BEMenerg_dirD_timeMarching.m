function density = BEMenerg_dirD_timeMarching(pbParam, domainMesh, methodInfo, GHn, GHw, GHCn, GHCw, DIAGn, DIAGw)
%
...
%Dimensioni problema
numBlocksV = BEMenerg_dirD_calcNumBlocks(pbParam, domainMesh);
...
constValues = BEMenerg_dirD_calcCostantData(methodInfo, domainMesh, GHn, GHw, GHCn, GHCw, DIAGn, DIAGw);
...
for indIter = 1 : numIterV
    ...
    matrixOut = BEMenerg_dirD_launchKernelV(gpuID, kernelV, deltaT, pbParam.velP, pbParam.velS, 4*pi*pbParam.rho, ...
    ...
    for indTemp = 1 : numBlocksPerIterV
        ...
        parfor indBlock = 1 : numTriang
            ...
            matrixSubBlocksDIAG(:, :, indBlock, indTemp) = BEMenerg_dirD_calcSubBlockDiagV(pbParam, methodInfo, istTemp, deltaT, constValues{indBlock}, DIAGn, DIAGw);
        end
    end
...
for indIter = 1 : numIterK
    %Avvio computazione GPU
    matrixOut = BEMenerg_dirD_launchKernelK(gpuID, kernelK, deltaT, pbParam.velP, pbParam.velS, pbParam.lambda, pbParam.mu, pbParam.rho, 4*pi*deltaT, ...
                                                    stdGHw, stdGHnx, stdGHny, stdGHnz, methodInfo.numNodiExt, ...
                                                    stdGHCw, stdGHCnx, stdGHCny, stdGHCnz, ...
    ...
    for indTemp = 1 : numBlocksPerIterK
        ...
        parfor indRow = 1 : numTriang
            for indV = 1 : 3
                matrixSubBlocksSING(:, :, indV, indRow, indTemp) = BEMenerg_dirD_calcSubBlockSingK(methodInfo, deltaT, pbParam, domainMesh, DIAGn, DIAGw, indTempCurr, indRow, constValues{indRow}, indV);
            end
        end
    end
...
betaK = BEMenerg_dirD_calcBetaKgpu(deltaT, pbParam, domainMesh, matrixSavedK);
