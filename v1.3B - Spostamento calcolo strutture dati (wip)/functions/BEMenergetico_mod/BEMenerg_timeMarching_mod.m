function [sol, timeVal] = BEMenerg_timeMarching_mod(pb_param, domainMesh, gha, ghw)
% INPUT 
...
tmpPath = strcat('./temp/', pb_param.domain_type, num2str(pb_param.lev));
outPath = strcat('./output/', pb_param.domain_type, num2str(pb_param.lev));

%% CALCOLO PARAMETRI COSTANTI
constValues = BEMenerg_calcCostantThings(pb_param, domainMesh, gha);

%% CALCOLO BLOCCO MATRICIALE E0 e BLOCCO VETTORIALE b0 

%Calcolo blocco matriciale E0
timeStd = cputime;
[matrix, ~] = BEMenerg_calcMatrixBlock_mod(matrix, hk, dt, pb_param, domainMesh, ghw, constValues);
timeEnd = cputime;
timeVal(1) = timeEnd - timeStd;
...
for ind_step = 2 : pb_param.Nt
    ...
    if(maxstep == 0)
        ...
        timeStd = cputime;
        [matrix, isnotzero] = BEMenerg_calcMatrixBlock_mod(matrix, hk, dt, pb_param, domainMesh, ghw, constValues);
        timeEnd = cputime;
        timeVal(ind_step) = timeEnd - timeStd;
