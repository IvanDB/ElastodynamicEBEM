...
addpath ./functions/BEMenergetico_mod
addpath ./functions/BEMenergetico_mod/integrazioneSingola
addpath ./functions/BEMenergetico_mod/integrazioneDoppia
addpath ./functions/BEMenergetico_mod/integrazioneDoppia/operazioniGeometriche
addpath ./functions/BEMenergetico_mod/integrazioneDoppia/integrazioniAnaliticheEsplicite/
addpath ./functions/BEMenergetico_mod/integrazioneDoppia/integrazioniAnaliticheEsplicite/coefficientiB/
addpath ./functions/BEMenergetico_mod/integrazioneDoppia/integrazioniAnaliticheEsplicite/coefficientiG/
...
tSTDstart = cputime;
[densitySTD, timersSTD] = BEMenerg_timeMarching(pb_param, domainMesh, gha, ghw);
tSTDend = cputime;

tMODstart = cputime;
[densityMOD, timersMOD] = BEMenerg_timeMarching_mod(pb_param, domainMesh, gha, ghw);
tMODend = cputime;

if(max(max(abs(densitySTD - densityMOD))) ~= 0)
    error("IL CODICE PARE SI SIA SBAGLIATO...")
end

%% DISPLAY TABLES
format shortG
disp(table(tSTDend - tSTDstart, tMODend - tMODstart, 'VariableNames', {'Tempo STD', 'Tempo MOD'}));
disp(table(timersSTD', timersMOD', 'VariableNames', {'Timers STD', 'Timers MOD'}));

%% PLOT DENSITY
% glb_index_figures = plotDensity(pb_param, domainMesh, densitySTD, glb_index_figures);
...
if(x_sz && t_sz)
    ...
    parfor ind = 1 : n_points
        ...
        sol_raw(ind, :) = postProcessing(pb_param, domainMesh, densitySTD, x_val(ind_x, :), t_val(ind_t));
    end
